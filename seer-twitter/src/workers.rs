...
use seer_common::{spawn_interval_worker, stop_active_worker, RUNNER_KIND_TWITTER};
    ...
    spawn_interval_worker(
        ...
        Duration::from_secs(secs),
        {
            ...
            move || {
                ...
                let name = name.clone();
                async move {
                    let log = match start_worker_run_log(&db, RUNNER_KIND_TWITTER, runner_id, &name)
                        .await
                    {
                        Ok(l) => l,
                        Err(e) => {
                    ...
                    if let Err(e) = finish_worker_run_log(&db, &log, err.as_deref()).await {
                        warn!(error = %e, "failed to finish twitter worker run log");
                    }
                }
            }
        },
