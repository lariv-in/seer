//! Twitter worker (runner) CRUD + start/stop pool.
...
use seer_common::{
    ...
    seer_table_with_actions, RUNNER_KIND_TWITTER,
};
use serde::Deserialize;

use crate::{
    entities::{
        twitter_runner::{self, ActiveModel as RunnerAM, Entity as RunnerEntity},
        twitter_source::{self, Entity as SourceEntity},
    },
    forms::WorkerForm,
    keys::{
        TwitterWorkerCreateModalKey, TwitterWorkerSelectModalKey, TwitterWorkerSelectTableKey,
        TwitterWorkersTableKey,
    },
    routes::{
        TwitterWorkerDetailRouteTag, TwitterWorkerEditGetRouteTag, TwitterWorkerPoolStartRouteTag,
        TwitterWorkerPoolStopRouteTag, TwitterWorkersListRouteTag,
    },
    state::TwitterState,
    templates::{
        TwitterWorkerCreatePage, TwitterWorkerDetailPage, TwitterWorkerEditPage,
        TwitterWorkerSelectPage, TwitterWorkersListPage,
    },
...
async fn source_items_for_runner(db: &DatabaseConnection, runner_id: i64) -> Vec<ManyToManyItem> {
    SourceEntity::find()
        .filter(twitter_source::Column::TwitterRunnerId.eq(runner_id))
...
async fn source_items_from_ids(db: &DatabaseConnection, ids: &[i64]) -> Vec<ManyToManyItem> {
    ...
    let rows = SourceEntity::find()
        .filter(twitter_source::Column::Id.is_in(ids.to_vec()))
        .all(db)
        .await
        .unwrap_or_default();
    ids.iter()
...
pub async fn sync_runner_sources(
    ...
    let assigned = SourceEntity::find()
        ...
        .await?;
    for s in assigned {
        let mut am: twitter_source::ActiveModel = s.into();
        am.twitter_runner_id = Set(None);
    ...
    for &sid in source_ids {
        ...
        if s.twitter_runner_id.is_some() {
            continue; // already owned by another runner
        }
        let mut am: twitter_source::ActiveModel = s.into();
        am.twitter_runner_id = Set(Some(runner_id));
        am.updated_at = Set(Some(Utc::now()));
        am.update(db).await?;
    }
    Ok(())
...
pub async fn list(
    ...
    let runners = RunnerEntity::find()
        .order_by_desc(twitter_runner::Column::Id)
        .all(&state.db)
        .await
        .unwrap_or_default();
    ...
    let rows: Vec<_> = runners
        .iter()
        .map(|r| {
            let href = TwitterWorkerDetailRouteTag::new(r.id).url();
            let status = if is_active_worker(RUNNER_KIND_TWITTER, r.id) {
                "Running"
            } else {
                "Stopped"
            };
    ...
    let actions = table_create_button::<TwitterWorkersTableKey, TwitterWorkerCreateModalKey>(
        Some("plus"),
        "btn-square btn-outline btn-sm",
    );
    let body =
        seer_table_with_actions::<TwitterWorkersTableKey>("Workers", actions, &headers, &rows);
    html_built_page_or_app_layout(
        &TwitterWorkersListPage { body },
        &htmx,
        &chrome,
...
pub async fn create_get(
    ...
    let page = TwitterWorkerCreatePage {
...
pub async fn create_post(
    ...
    respond_create_modal_done_fk::<TwitterWorkerCreateModalKey>(
        &htmx,
        &q.refresh_table(),
        &TwitterWorkerDetailRouteTag::new(row.id).url(),
        row.id,
        &row.name,
        &q.target_input(),
    )
...
pub async fn detail(
    ...
    let Some(r) = lariv_rs::web::opt_or_log(
        ...
        return Redirect::to(&TwitterWorkersListRouteTag.url()).into_response();
    };
    let sources = SourceEntity::find()
        .filter(twitter_source::Column::TwitterRunnerId.eq(id))
        .order_by_desc(twitter_source::Column::Id)
        .all(&state.db)
        .await
        .unwrap_or_default();
    let running = is_active_worker(RUNNER_KIND_TWITTER, id);
    let edit_url = TwitterWorkerEditGetRouteTag::new(id).url();
    let pool_btn = if running {
        button_post_route(
            TwitterWorkerPoolStopRouteTag::new(id),
            "Stop worker pool",
            "btn-warning",
        )
    } else {
        button_post_route(
            TwitterWorkerPoolStartRouteTag::new(id),
            "Start worker pool",
            "btn-success",
        )
    };
    let body = html! {
        div class="container mx-auto p-4 space-y-4" {
            div class="flex flex-wrap gap-2" {
                (pool_btn)
                (button_link_route(TwitterWorkerEditGetRouteTag::new(id), "Edit", "btn-outline"))
                form method="post" action=(format!("/seer-twitter/workers/{id}/delete"))
...
pub async fn edit_get(
    ...
    let page = TwitterWorkerEditPage {
        id,
        name: r.name,
...
pub async fn edit_post(
    ...
    if is_active_worker(RUNNER_KIND_TWITTER, id) {
        start_runner_pool(state.db.clone(), id, name, duration_secs);
    }
    htmx.redirect(&TwitterWorkerDetailRouteTag::new(id).url())
}
...
pub async fn delete_post(
    ...
    if linked > 0 {
        ...
        return htmx.redirect(&TwitterWorkerDetailRouteTag::new(id).url());
    }
    stop_runner_pool(id);
    let _ = RunnerEntity::delete_by_id(id).exec(&state.db).await;
    htmx.redirect(&TwitterWorkersListRouteTag.url())
}
...
pub async fn select(
    ...
    let mut query = RunnerEntity::find().order_by_desc(twitter_runner::Column::Id);
    if let Some(name) = q.name.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        query = query.filter(twitter_runner::Column::Name.contains(name));
    }
    let runners = query.limit(50).all(&state.db).await.unwrap_or_default();
    let items: Vec<_> = runners.into_iter().map(|r| (r.id, r.name)).collect();
    ...
    let page = TwitterWorkerSelectPage {
        runners: list,
        filter_name: q.name.clone().unwrap_or_default(),
        target_input: q.target_input.unwrap_or_else(|| "twitter_runner_id".into()),
        path_and_query: path_and_query(&uri),
    };
    respond_picker_select::<TwitterWorkerSelectTableKey, TwitterWorkerSelectModalKey, _>(
        &htmx, &page,
    )
    .into_response()
}
